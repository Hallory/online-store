...
const MainProductsDescriptions = ({ product }) => {
    ...
    return (
        <div className="w-[39rem] max-h-[30rem] lg:min-h-[25rem] flex flex-col rounded-md ">
            <div className="w-full flex gap-2 items-center border-b border-gray-300  ">
                <button
                    className={`p-3 rounded-t-md ${
                        mainDetails === 'performance' ? 'bg-primary-300' : ''
                    }`}
                    onClick={() => setMainDetails('performance')}
                >
                    Performance
                </button>
                <button
                    className={`p-3 rounded-t-md ${
                        mainDetails === 'features' ? 'bg-primary-300' : ''
                    }`}
                    onClick={() => setMainDetails('features')}
                >
                    Features
                </button>
                <button
                    className={`p-3 rounded-t-md ${
                        mainDetails === 'design' ? 'bg-primary-300' : ''
                    }`}
                    onClick={() => setMainDetails('design')}
                >
                    Design
                </button>
                <button
                    className={`p-3 rounded-t-md ${
                        mainDetails === 'description' ? 'bg-primary-300' : ''
                    }`}
                    onClick={() => setMainDetails('description')}
                >
                    Description
                </button>
            </div>
            ...
            <div
                className={`w-full flex pl-2 rounded-md flex-col  ${
                    mainDetails === 'performance' ? 'rounded-tl-none' : ''
                }`}
            >
                {handleMainDetails()}
            </div>
